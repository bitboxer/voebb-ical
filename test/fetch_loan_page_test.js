...
import fetchLoanPage from '../src/fetch_loan_page.mjs';
...
describe('FetchLoanPage', () => {
  it('should load the loan page of an user', async () => {
    ...
    const result = await fetchLoanPage('veobid', 'password', (url, params) => mock.fetch(url, params));
  ...
  it('should throw an error if the username/password is wrong', () => {
    ...
    expect(fetchLoanPage('veobid', 'password', (url, params) => mock.fetch(url, params))).to.be.rejectedWith('Login failed. Please check username and password.');
  }).timeout(60000);
});
