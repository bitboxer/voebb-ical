...
import fetchLoanPage from '../src/app/fetch_loan_page.mjs';
...
describe('FetchLoanPage', () => {
  it('should load the loan page of an user', async () => {
    ...
    mock.addPage(fs
      ...
      .toString());
    mock.addPage(fs.readFileSync('./test/data/fetch_loan_page/5_loans.html').toString());
  ...
  it('should throw an error if the username/password is wrong', () => {
    ...
    expect(fetchLoanPage('veobid', 'password', (url, params) =>
      mock.fetch(url, params))).to.be.rejectedWith('Login failed. Please check username and password.');
  }).timeout(60000);
});
